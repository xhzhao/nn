...
#define TH_GENERIC_FILE "generic/LRNMKLDNN.c"
#else
...
static void THNN_(CrossChannelLRN_MKLDNN_init_forward)(
...
static void THNN_(CrossChannelLRN_MKLDNN_init_backward)(
          THLongTensor *primitives,
          int N,
          int outC,
          int outH,
	  ...
	  int size, float alpha, float beta, float k)
{
	...
	CHECK_ERR( THNN_(init_conversion)(&cv_backward_output, &buffer_backward_output, lt_lrn_backward_output, lt_user_output), err );
...
void THNN_(CrossChannelLRN_MKLDNN_updateOutput)(
  ...
  THLongTensor *primitives,int initOk)
{
	...
	if(initOk == 0)
	{
		...
		THNN_(CrossChannelLRN_MKLDNN_init_forward)(primitives,N,inC,inH,inW,size,alpha,beta,k);
	}
...
void THNN_(CrossChannelLRN_MKLDNN_backward)(
  ...
  THLongTensor *primitives,int initOk)
{
	...
	if(initOk == 0)
	{
		...
		THNN_(CrossChannelLRN_MKLDNN_init_backward)(primitives,N,outC,outH,outW,size,alpha,beta,k);
	}
